...
let rec perm_filter' allperms = function
  | A_NAME name -> 
...
(* let eval_classpermission_nomapping cpmex clss cpms =
  match cpmex with
    | Name qn -> 
        let cpname = List.nth qn 0 in
        SM.find cpname cpms 
    | Anonym (qn, clsperm) ->
        let classname = List.nth qn 0 in
        let allperms = (SM.find classname clss) in
